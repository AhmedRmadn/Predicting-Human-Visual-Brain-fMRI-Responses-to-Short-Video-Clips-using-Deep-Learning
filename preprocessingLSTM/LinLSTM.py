...
class StatsModel(nn.Module):
    ...
    def forward(self, seq):
        ...
        return torch.cat([seq[:, -1, :],seq[:, 0, :], embed_min, embed_max, embed_avg, embed_std], dim=1)
...
class RnnModel(nn.Module):
    ...
    def forward(self, input):
        h0_1 = torch.zeros(1, input.size(0), self.rnn_size).to(self.device) 
        c0_1 = torch.zeros(1,input.size(0), self.rnn_size).to(self.device) 

        h0_2 = torch.zeros(2, input.size(0), self.rnn_size).to(self.device) 
        c0_2 = torch.zeros(2,input.size(0), self.rnn_size).to(self.device) 

        h0_3 = torch.zeros(4, input.size(0), self.rnn_size).to(self.device) 
        c0_3 = torch.zeros(4,input.size(0), self.rnn_size).to(self.device) 

        RNN_out_1, _ = self.rnn_1(self.dropout(input),(h0_1,c0_1))
        RNN_out_2, _ = self.rnn_2(self.dropout(input),(h0_2,c0_2))
        RNN_out_3, _ = self.rnn_3(self.dropout(input),(h0_3,c0_3))
...
class Vid2FMRIModel(nn.Module):
    def __init__(self, num_of_features,embed_size, output_size=256, rnn_features=True, dropout_rate=0.2 ,rnn_size=1024):
